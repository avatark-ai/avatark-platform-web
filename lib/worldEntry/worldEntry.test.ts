...
test("gateway: malformed tickets never reach the database; cookie is HttpOnly/Secure/Lax; leave is same-origin only", async () => {
  ...
  assert.equal(page.headers.get("referrer-policy"), "same-origin", "the Leave form's page must let the browser send its real Origin")
  const nullOrigin = await handleLeave(new Request(`${ORIGIN}/world-entry/session/leave`, { method: "POST", headers: { cookie, origin: "null", "sec-fetch-site": "same-origin" } }), { db })
  assert.equal(nullOrigin.status, 403, "Origin: null stays refused")
