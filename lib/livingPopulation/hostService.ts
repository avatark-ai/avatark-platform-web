...
import type { AdvancePopulationSimulationResult } from "@avatark/living-population-runtime"
...
export async function advancePopulationForWorld(worldInstanceId: WorldInstanceId, sharedStateAtStart: SharedWorldState, ticks: number, seed: string, now: () => string): Promise<AdvancePopulationSimulationResult> {
  await ensureSeeded(worldInstanceId)
...
export async function wakeWorldWithPopulation(worldInstanceId: WorldInstanceId, ownerId: WorldOwnerId, now: () => string = () => new Date().toISOString()): Promise<WakeWorldWithPopulationResult> {
  const stateBeforeWake = await getWorldState(worldInstanceId, now)
  const world = await wakeWorld(worldInstanceId, ownerId, now)
  const population = await advancePopulationForWorld(worldInstanceId, stateBeforeWake.sharedState, world.ticksApplied, worldInstanceId, now)
