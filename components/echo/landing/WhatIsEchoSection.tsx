...
export function WhatIsEchoSection({ headline, paragraphs }: { headline: string; paragraphs: string[] }) {
  return (
    <section id="what-is-an-echo">
      <div className="mx-auto max-w-3xl px-6 py-20 text-center sm:py-24">
        <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl" style={{ color: "var(--paper)" }}>
          {headline}
        </h2>
        <div className="mx-auto mt-6 flex max-w-xl flex-col gap-3 text-lg leading-8" style={{ color: "var(--text-dim)" }}>
          {paragraphs.map((paragraph) => (
            <p key={paragraph}>{paragraph}</p>
          ))}
        </div>

        <ol className="mt-12 flex flex-wrap items-center justify-center gap-x-3 gap-y-3 text-sm font-medium" style={{ color: "var(--paper)" }}>
